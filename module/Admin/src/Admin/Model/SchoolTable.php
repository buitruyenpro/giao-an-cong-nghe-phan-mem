...
class SchoolTable extends ActionController
{
		...
		if ($options['task'] == 'list-item') {
			...
			$result = $this->tableGateway->select(function (Select $select) use ($arrParam) {
				...
				$select->columns(array(
					'id', 'name', 'status', 'ordering', 'created', 'created_by', 'modified', 'modified_by', 'group_acp', 'node_id'
				))
	...
	public function saveItem($arrParam = null, $options = null)
	{
		...
		if ($options['task'] == 'add-item') {
			$dataNode = ['name' => $arrParam['name'], 'parent' => '1', 'status' => ($arrParam['status'] == 'active') ? 1 : 0, 'level' => 2];
			$table    = $this->getServiceLocator()->get('Admin\Model\NestedPermissionTable');
			$NodeId   = $table->insertNode($dataNode, 1, array('position' => 'right'));
			$data     = array(
				'name'     => $arrParam['name'],
