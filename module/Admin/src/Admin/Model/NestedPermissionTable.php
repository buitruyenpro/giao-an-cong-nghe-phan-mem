<?php
	...
	class NestedPermissionTable extends AbstractTableGateway{
		...
		public function itemInSelectbox($arrParam = null, $options = null, $node_id = null, $ordering = null){
			if ($ordering != 1 && $ordering != 3 && $ordering != 4){
				if ($options['task'] == 'list-school'){
					
					$items  = $this->tableGateway->select(function (Select $select) use ($arrParam){
						$select->columns(array('id', 'name', 'level'))
						       ->order('left ASC')
							->where->greaterThan('left', 0)->and->equalTo("level", 1);
					});
					$result = array();
					if (!empty($items)){
			...
			if ($options['task'] == 'list-nest'){
				if ($ordering != 1 && $ordering != 2 && $ordering != 4){
					//Lấy thông tin node hiện tại
					$result = $this->tableGateway->select(function (Select $select) use ($arrParam, $node_id){
						$select->columns(array('id', 'name', 'level', 'left', 'right', 'parent'))
							->where->equalTo('id', $node_id);;
					})->current();
					
					//Lấy node con
					$arrParam['data']['catIDs'] = $this->listNodes($result, array('task' => 'list-branch-nest'));
					
					
					$items = $this->tableGateway->select(function (Select $select) use ($arrParam){
						$select->columns(array('id', 'name', 'level'))
						       ->order('left ASC')
							->where->in('id', $arrParam['data']['catIDs']);
					});
					
					$result = array();
					if (!empty($items)){
						foreach ($items as $item){
							$result[$item->id] = str_repeat('------|', $item->level - 2) . ' ' . $item->name;
						}
					}
				}
				return $result;
		...
		public function listNodes($arrParam = null, $options = null){
			...
			if ($options['task'] == 'list-branch-nest'){
				$items = $this->tableGateway->select(function (Select $select) use ($arrParam){
					$select->columns(array('id', 'name', 'level', 'parent'))
					       ->order('left ASC')
		...
		public function getNodeId($arrParam, $options = null){
			return null;
		}
		...
		public
		function removeNodeOnly($nodeID){
			$nodeInfo = $this->getNodeInfo(array('id' => $nodeID));
