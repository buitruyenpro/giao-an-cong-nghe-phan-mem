...
class NestTable extends ActionController
{
	...
	public function countItem($arrParam = null, $options = null)
	{
		...
		if ($options['task'] == 'nest-in-school') {
			...
			$result = $this->tableGateway->select(function (Select $select) use ($arrParam) {
				...
				$select->where->in('id_node', $arrParam['data']['catIDs']);
			})->count();
		}
		...
		if ($options['task'] == 'list-item') {
			...
			$result = $this->tableGateway->select(function (Select $select) use ($arrParam) {
				...
				$select->columns(array(
					'id', 'name', 'status', 'ordering', 'created', 'created_by', 'modified', 'modified_by', 'group_acp','id_node'
				))
		...
		if ($options['task'] == 'nest-in-school') {
			...
			$result = $this->tableGateway->select(function (Select $select) use ($arrParam) {
				...
				$select->columns(array(
					...
					->limit($paginator['itemCountPerPage'])
					->offset(($paginator['currentPageNumber'] - 1) * $paginator['itemCountPerPage'])
					->where->in('id_node', $arrParam['data']['catIDs']);
	...
	public function saveItem($arrParam = null, $options = null, $id_permission_nested = null)
	{

		if ($options['task'] == 'add-item') {

			$dataNode = ['name' => $arrParam['name'], 'parent' => $id_permission_nested, 'status' => ($arrParam['status'] == 'active') ? 1 : 0, 'level' => 2];
			$table    = $this->getServiceLocator()->get('Admin\Model\NestedPermissionTable');

			$NodeId = $table->insertNode($dataNode, $id_permission_nested, array('position' => 'right'));
			$data   = array(
				'name'     => $arrParam['name'],
		...
		if ($options['task'] == 'edit-item') {
			...
			$table = $this->getServiceLocator()->get('Admin\Model\NestedPermissionTable');
			$table->updateNode($data, $arrParam['id_node']);
			return $arrParam['id'];
		}
	}
}