<?php
	...
	class GroupTable extends AbstractTableGateway
	{
		...
		public function itemInSelectbox($arrParam = null, $options = null, $ordering = null)
		{
			if ($options == null) {
			...
			if ($options['task'] == 'form-user') {
				$items  = $this->tableGateway->select(function (Select $select) use ($arrParam, $ordering) {
					$select->columns(array('id', 'name'))
						->order('ordering DESC')
						->where->equalTo('status', 1)
						->and->greaterThanOrEqualTo('ordering', $ordering);
		...
		public function getIDPermision($arrParam = null, $options = null)
		{
			if ($arrParam ==null) {
				return null;
			}
			...
			return Json::decode($result->permission_id);
		...
		public function getOrdering($arrParam = null, $ordering = null)
		{
		...
		public function saveItem($arrParam = null, $options = null, $name)
		{
			...
			if ($options['task'] == 'add-item') {
				$data = array(
					...
					'permission_id' => Json::encode($arrParam['permission_id']),
					'created_by'    => $name
				);
