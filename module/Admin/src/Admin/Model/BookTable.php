...
class BookTable extends AbstractTableGateway {
		...
		if($options['task'] == 'list-item') {
			...
			$result	= $this->tableGateway->select(function (Select $select) use ($arrParam){
				...
				$select->columns(array(
							'id', 'name', 'status', 'picture', 'created', 'created_by', 'modified', 'modified_by', 'ordering', 'sale_off', 'special','document'
						))
						->join(
							array('c' => 'category'),
	...
	public function getItem($arrParam = null, $options = null){
		...
		if($options == null) {
			$result	= $this->tableGateway->select(function (Select $select) use ($arrParam){
					$select->columns(array('id', 'name', 'ordering', 'picture','status', 'special','category_id', 'description', 'sale_off','document'));
