<?php
	...
	class NestController extends ActionController{
		public function init(){
			...
			// OPTIONS
			$this->_options['tableName'] = 'Admin\Model\NestTable';
			$this->_options['formName']  = 'formAdminNest';
		...
		public function indexAction(){
			...
			if ($this->_params['data']['id']){
				...
				// LIST BOOK
				$this->_params['data']['catIDs'] = $nodeTable->listNodes($categoryInfo, array('task' => 'list-branch-nest'));
				
				$total = $this->getTable()->countItem($this->_params, array('task' => 'nest-in-school'));
				$items = $this->getTable()->listItem($this->_params, array('task' => 'nest-in-school'));
			}else{
		...
		public function formAction(){
			$id_permission_nested = $this->identity()->id_permission_nested;
			...
			if ($this->getRequest()->isPost()){
				...
				if ($myForm->isValid()){
					...
					$id                    = $this->getTable()->saveItem($this->_params['data'], array('task' => $task), $id_permission_nested);
