...
class Image {
	...
	public function upload($fileInput, $options = null){
		...
		if($options['task'] == 'book-picture'){
			...
			$layer->save(PATH_FILES . '/books/thumb/180x270', $fileName, true);
		...
		if($options['task'] == 'book-document'){
			$uploadObj 			= new Upload();
			$uploadDirectory	= PATH_FILES . '/documents/';
			$fileName			= $uploadObj->uploadFile($fileInput, $uploadDirectory, array('task' => 'rename'), 'document_');
	...
	public function removeImage($fileName, $options = null){
		...
		if($options['task'] == 'book-picture'){
			...
			$fileThumb	= PATH_FILES . '/books/thumb/180x270/' . $fileName;
			@unlink($fileThumb);
